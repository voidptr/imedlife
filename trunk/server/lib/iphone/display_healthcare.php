<?php
//display_healthcare.php - Renders the healthcare provider information to the iPhone upon the request.
...
if(isset($_GET['sessionID'])) {
	...
	if($loggedIn == true) {
		...
		$patientID = $row[0];
			...
			$query = "SELECT * FROM healthcareProviders WHERE patientID='$patientID'";
			$result = mysql_query($query); //Run the query
			$columns = mysql_query("SHOW COLUMNS FROM healthcareProviders");	
